...
from main import create_access_token, JWT_SECRET, JWT_ALGORITHM, JWT_EXPIRY_MINUTES
...
class TestCreateAccessToken:
    ...
    def test_create_access_token_empty_payload(self):
        """Token with empty data should still contain exp only."""
        token = create_access_token({})
        decoded = jwt.decode(token, JWT_SECRET, algorithms=[JWT_ALGORITHM])

        assert "exp" in decoded
        assert len(decoded) == 1  # Only exp field exists

    def test_create_access_token_with_special_characters(self):
        """Test UTF-8 payload handling."""
        complex_data = {
            "sub": "user@example.com",
