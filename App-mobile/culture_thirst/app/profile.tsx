  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      if (!user) {
        ...
        // Charger les poèmes débloqués
        const userPoemsRef = ref(db, `userPoems/${user.uid}`);
        const poemsListener = onValue(userPoemsRef, async (snapshot) => {
          const userPoemsData = snapshot.val();
          if (userPoemsData) {
            const poemIds = Object.keys(userPoemsData);
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.content}>
        ...
        <Text style={styles.sectionTitle}>Poèmes débloqués ({userData?.unlockedPoemsCount || 0})</Text>
        {poems.length > 0 ? (
          <FlatList
            data={poems}
